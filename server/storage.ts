...
export class MongoStorage implements IStorage {
  ...
  async getUserByMobile(mobile: string): Promise<User | undefined> {
    // Normalize: strip spaces, country code prefix, leading zero
    const clean = mobile.replace(/\s/g, "").replace(/^\+?966/, "").replace(/^0/, "");
    // Match any stored format: with spaces, without, with leading zero, etc.
